...
def ifft(fshift):
    ...
    iimg_red_yellow = np.zeros((iimg.shape[0], iimg.shape[1], 3), dtype=np.uint8)
    iimg_red_yellow[:, :, 0] = iimg  # 设置红色通道
    iimg_red_yellow[:, :, 1] = iimg  # 设置绿色通道
    iimg_red_yellow[:, :, 2] = 0  # 设置蓝色通道为零
    #return iimg
    return iimg_red_yellow
...
if __name__ == '__main__':
    ...
    D = 10      # 高斯过滤器的截止频率：2 5 10 20 50 ，越小越模糊信息越少
    img = cv2.imread('02617.png', cv2.IMREAD_GRAYSCALE)
    print(img[0][0])
    low_freq_part_img, high_freq_part_img = get_low_high_f(img, radius_ratio=radius_ratio, D=D)  # multi channel or single
    #print(high_freq_part_img[100])
    print(high_freq_part_img.shape)
    plt.subplot(141), plt.imshow(img, 'gray'), plt.title('Original Image')
    plt.axis('off')
    plt.subplot(142), plt.imshow(low_freq_part_img, 'gray'), plt.title('low_freq_img')
    plt.axis('off')
    plt.subplot(143), plt.imshow(high_freq_part_img, 'gray'), plt.title('high_freq_img')
    plt.axis('off')
    plt.show()
    result = cv2.add(low_freq_part_img, high_freq_part_img)
    plt.subplot(144), plt.imshow(result, 'gray'), plt.title('high_freq_img')
    plt.axis('off')
    cv2.imwrite("result.png",high_freq_part_img) 
